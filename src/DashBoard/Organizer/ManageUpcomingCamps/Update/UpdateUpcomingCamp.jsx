...
// const UpdateUpcomingCamp = () => {
    const UpdateUpcomingCamp = () => {
        ...
        const { data: singleCamp = [], isLoading, refetch } = useQuery({
            queryKey: [`singleupcomingcampupdatedetails${campId}`, user],
            
            enabled: !!user?.email && !!localStorage.getItem('token'),
            queryFn: async () => {
                const res = await axiosSecure.get(`/upcommingcamps/${campId}`)
                return res?.data
            }
        })
        useEffect(() => {
      ...
      const   time = (gettime) => {
          ...
          const loadingfromimgbb = async (image) => {
            try {
        ...
        return (
            <div className='' style={{ background: `url(${singleCamp?.image})`, backgroundSize: 'cover', backgroundRepeat:'no-repeat',  backgroundAttachment: 'fixed' }}>
                <div className='w-full   bg-[#00000069] py-10'>
                <Helmet>
                    <title>Mediserve Mobilize |  Update upcoming Camp</title>
                </Helmet>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className=' p-1 sm:p-4 md:w-[80%] mx-auto bg-[#ffffffa2] rounded-t-lg'>
                            <Title title={'Update Camp'} desc={'Update Upcoming Camp You Want'}></Title>
                        </div>
